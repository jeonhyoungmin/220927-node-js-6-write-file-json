...
// pokemonObj JSON 저장하기 위해 수집한 객체방식의 데이터인 것을 가정한다.
const pokemonJSON = JSON.stringify(pokemonObj, null, 2);
// 위 pokemonObj를 일련의 가공처리(JSON 포멧)로 처리한 값을 pokemonJSON으로 대입했다.
// stringify(객체, 인덴팅 방식, 칸)
fs.writeFile('./pokemonJSON.json', pokemonJSON, (err) => {
  if(err) throw err;
});
// file system에서 지원하는 파일 쓰기(write) === 파일 만들기 함수를 통해,
// 해당 데이터를 json방식으로 저장했다.
